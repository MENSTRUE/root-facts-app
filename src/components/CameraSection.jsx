...
function CameraSection({
  ...
  useEffect(() => {
    ...
    if (
      videoRef.current
      && services.camera.video !== videoRef.current
    ) {
    ...
    if (
      canvasRef.current
      && services.camera.canvas !== canvasRef.current
    ) {
      services.camera.setCanvasElement(canvasRef.current);
    }
  });

  const handleCameraFacingChange = (event) => {
    const facingMode = (
  ...
  const handleFpsChange = (event) => {
    onFpsChange?.(Number(event.target.value));
  };
  ...
  return (
    <section
      ...
      aria-label="Camera Feed and Controls"
    >
        ...
        <div className="settings-bar">
          ...
          <div className="setting-item tone-setting">
            ...
            <select
              ...
              value={
                currentTone
                || TONE_CONFIG.defaultTone
              }
              onChange={handleToneChange}
              aria-label="Pilih persona fakta menarik"
            >
              {TONE_CONFIG.availableTones.map(
                (option) => (
                  <option
                    key={option.value}
                    value={option.value}
